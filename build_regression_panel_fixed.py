...
try:
    ...
    add_report('专利申请数据', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 财政收入：{len(fiscal_rev)} 条记录")
    add_report('财政收入', '[OK]', len(fiscal_rev), 
               f"{fiscal_rev['年份'].min()}-{fiscal_rev['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] 财政收入：{e}")
    add_report('财政收入', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 财政支出：{len(fiscal_exp)} 条记录")
    add_report('财政支出', '[OK]', len(fiscal_exp), 
               f"{fiscal_exp['年份'].min()}-{fiscal_exp['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] 财政支出：{e}")
    add_report('财政支出', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 政府债务：{len(debt)} 条记录")
    add_report('政府债务', '[OK]', len(debt), 
               f"{debt['年份'].min()}-{debt['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] 政府债务：{e}")
    add_report('政府债务', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 科技支出：{len(tech_exp)} 条记录")
    add_report('科技支出', '[OK]', len(tech_exp), 
               f"{tech_exp['年份'].min()}-{tech_exp['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] 科技支出：{e}")
    add_report('科技支出', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] GDP：{len(gdp)} 条记录")
    add_report('GDP', '[OK]', len(gdp), 
               f"{gdp['年份'].min()}-{gdp['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] GDP：{e}")
    add_report('GDP', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 人均GDP：{len(gdp_pc)} 条记录")
    add_report('人均GDP', '[OK]', len(gdp_pc), 
               f"{gdp_pc['年份'].min()}-{gdp_pc['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] 人均GDP：{e}")
    add_report('人均GDP', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 第二产业：{len(industry)} 条记录")
    add_report('第二产业', '[OK]', len(industry), 
               f"{industry['年份'].min()}-{industry['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] 第二产业：{e}")
    add_report('第二产业', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 常住人口：{len(pop)} 条记录")
    add_report('常住人口', '[OK]', len(pop), 
               f"{pop['年份'].min()}-{pop['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] 常住人口：{e}")
    add_report('常住人口', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 实际利用外资：{len(fdi)} 条记录")
    add_report('实际利用外资', '[OK]', len(fdi), 
               f"{fdi['年份'].min()}-{fdi['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] 实际利用外资：{e}")
    add_report('实际利用外资', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 金融机构贷款：{len(finance)} 条记录")
    add_report('金融机构贷款', '[OK]', len(finance), 
               f"{finance['年份'].min()}-{finance['年份'].max()}", '宽格式转换')
except Exception as e:
    print(f"[FAIL] 金融机构贷款：{e}")
    add_report('金融机构贷款', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    if gf_list:
        ...
        print(f"[OK] 引导基金投资：{len(gf)} 条记录")
        print(f"  （需要进一步处理：匹配城市、计算早期投资占比）")
        add_report('引导基金投资', '[OK]', len(gf), '-', '需进一步处理匹配城市')
    else:
        print("[FAIL] 未能读取引导基金数据")
        add_report('引导基金投资', '[FAIL]', 0, '-', '文件读取失败')
        
except Exception as e:
    print(f"[FAIL] 引导基金投资：{e}")
    add_report('引导基金投资', '[FAIL]', 0, '-', str(e))
...
try:
    ...
    print(f"[OK] 市场化指数：{len(market_idx)} 条记录")
    print(f"  （需要进一步处理：省级数据匹配到城市）")
    add_report('市场化指数', '[OK]', len(market_idx), '-', '省级数据，需匹配到城市')
    
except Exception as e:
    print(f"[FAIL] 市场化指数：{e}")
    add_report('市场化指数', '[FAIL]', 0, '-', str(e))
...
print(f"[OK] 完整面板已保存：cleaned_data/final_regression_panel.csv")
...
print(f"[OK] 含滞后项面板已保存：cleaned_data/final_regression_panel_with_lags.csv")
...
print("[OK] 数据质量报告已保存：cleaned_data/data_quality_report.csv")
print()

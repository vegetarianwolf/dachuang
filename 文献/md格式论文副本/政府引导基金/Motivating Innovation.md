...
∗MIT Sloan School of Management, 50 Memorial Dr E52-446, Cambridge, MA 02142 (e-mail:
manso@mit.edu, url: www.mit.edu/∼manso/). I thank John Roberts, Anat Admati, Darrell Duﬃe,
and Ed Lazear for valuable discussions and suggestions. I also thank Peter DeMarzo, Bob Gibbons,
Thomas Hellman, Bengt Holmstrom, Jim March, Pedro Miranda, Stew Myers, Tomasz Sadzik, Maria
Salgado, Yuliy Sannikov, Antoinette Schoar, Andy Skrzypacz, Ilya Strebulaev, Alexei Tchistyi, Jeﬀ
Zwiebel and seminar participants at Berkeley, Chicago, Columbia, Duke, FGV, Harvard, LBS, LSE,
MIT, Northwestern, NYU, Princeton, PUC-Rio, Stanford, UCLA, UCSD, UIowa, University of Oregon,
and UPenn for helpful comments and Forrest Funnell for outstanding research assistance. Financial
support from a Stanford Institute for Policy Research (SIEPR) Fellowship and the Lehman Brothers
Fellowship for Research Excellence in Finance is gratefully acknowledged.

Electronic copy available at: https://ssrn.com/abstract=891514
Electronic copy available at: http://ssrn.com/abstract=891514
Electronic copy available at: http://ssrn.com/abstract=891514

Berle and Means’ (1932) seminal contribution brought to light the potential draw-
...
manage resources to his own advantage. To alleviate possible conﬂicts of interest between
...
principal oﬀers the agent an incentive plan to induce the agent to act in the principal’s

best interest. Most of the papers in this literature focus on the problem of inducing

the agent to exert eﬀort or avoiding the agent from tunneling resources away from the

corporation.

Here I study a diﬀerent problem: how to structure incentives when the principal needs
...
innovation should be structured diﬀerently from standard pay-for-performance schemes

used to induce eﬀort or to avoid tunneling. Innovation involves the exploration of new
...
eﬀects on innovation. In contrast, the optimal incentive scheme that motivates inno-
...
may keep his job even if it is ex-post eﬃcient for the shareholders of the ﬁrm to ﬁre him.
...
to exert eﬀort. Occasionally there are proposals to adopt regulations that restrict the use
...
use may thus have an adverse eﬀect on innovation. In order to assess the actual impact
...
of payoﬀs of the available actions. Innovation in this setting is the discovery, through
...
but is also likely to waste time with inferior actions. Exploitation of well known actions

ensures reasonable payoﬀs, but may prevent the discovery of superior actions.
...
for example, between shareholders of a ﬁrm designing the compensation package oﬀered

to a manager, the manager of a ﬁrm designing an incentive plan for a worker, or a

venture capitalist ﬁnancing an entrepreneur.
...
to the agent, there is no conﬂict of interest between the principal and the agent. The
...
dard principal-agent model where the principal must motivate the agent to exert eﬀort.
...
tally diﬀerent from each other. Since exploitation is just the repetition of well known

actions, the optimal contract that motivates exploitation is similar to standard pay-for-

performance contracts used to motivate repeated eﬀort. On the other hand, since with
...
The threat of termination after poor performance also aﬀects the incentives for ex-
...
In contrast, the eﬀects of termination on the incentives for exploration are ambiguous.
...
of these two eﬀects is more important, termination may either facilitate or hinder the
...
diﬀer depending on whether the principal wants to motivate exploration or exploitation.
...
governance and executive compensation, bankruptcy laws and entrepreneurship, and the
...
process explicitly and focuses on the central trade-oﬀ that arises in innovation activities,
...
and Smith (2001), has relied extensively on bandit problem and related models of learn-
...
not limited to the level of eﬀort. Holmstrom and Milgrom (1991) develop a multi-task

principal-agent model in which the agent allocates eﬀort across multiple tasks and the
...
rent model resembles the multi-task model in that the agent can choose to allocate eﬀort
...
1995). Graham, Harvey, and Rajgopal (2005) ﬁnd, in a survey of ﬁnancial executives,

6

Electronic copy available at: https://ssrn.com/abstract=891514

that the majority of managers would pass a positive NPV project to avoid missing the

current quarter’s consensus earnings forecast. Using ﬁrm-level data, Dechow and Sloan

(1991) and Bushee (1998) ﬁnd that short-termism is more prevalent for CEOs near

retirement and in ﬁrms held by transient institutional investors.

To study the ﬁnancing of innovation, Bergemann and Hege (2005) develop a principal-
...
Some contemporaneous papers ﬁnd empirical support for a few of the results de-

rived here. For example, Acharya and Subramanian (2009) ﬁnd that debtor-friendly

bankruptcy laws lead to more innovation. Acharya, Baghai-Wadji, and Subramanian

(2009) ﬁnd that stringent labor laws that restrict the dismissal of employees encourage

ﬁrm-level innovation. Francis, Hasan, and Sharma (2009) show that golden parachutes

as well as long-term incentives in the form of vested and unvested options have a pos-

itive and signiﬁcant eﬀect on patents and citations to patents. Tian and Wang (2010)

ﬁnd that ﬁrms backed by venture capitalists that tolerate failures are signiﬁcantly more
...
resources by headquarters. Azoulay, Graﬀ Zivin, and Manso (2010) show that funding

policies with tolerance for early failure and long horizons to evaluate results motivate

creativity in scientiﬁc research. In a laboratory experiment, Ederer and Manso (2010)
...
More generally, the paper is related to the literature that studies the eﬀect of insti-

tutions on ﬁnancial organization and economic growth. Previous work in this literature

has documented the eﬀect of the institutional environment (e.g. corporate governance,

investor protection, political risk) on ﬁnancial development (La Porta, Lopez-de Silanes,
...
Howitt 1992), the ﬁndings of the current paper complement this literature and provide
...
The agent lives for two periods.

In each period, the agent takes an action i ∈
...
thus chooses an action plan hi

j

k i to maximize his total expected payoﬀ
...
+ (1 − E[pi]) {E[pk|F, i]S + (1 − E[pk|F, i])F } ,
...
where i ∈ I is the ﬁrst-period action, j ∈ I is the second-period action in case of success

in the ﬁrst period, and k ∈ I is the second-period action in case of failure in the ﬁrst

period.

Two action plans need to be considered. Action plan h1 1

1 i, which I call exploitation,

is just the repetition of the conventional work method. Action plan h2 2

1i, which I call

exploration, is to initially try the new work method, stick to the new work method in

case of success in the ﬁrst period, and revert to the conventional work method in case of

failure in the ﬁrst period. The total payoﬀ R(h2 2

1 i) from exploration is higher than the

total payoﬀ R(h1 1

1 i) from exploitation if and only if
...
ﬁrst-period payoﬀ that the agent is willing to pay to obtain information about p2.

The agent is willing to sacriﬁce more in the ﬁrst period if he lives for multiple periods.

With multiple periods, the beneﬁts of experimenting with the new work method are
...
team. In a team, the optimal action plan involves more sacriﬁcing of ﬁrst period output

for at least one of the agents. In case the agent discovers that the new work method is

better than the conventional work method, the whole team beneﬁts from his discovery.
...
work method, perhaps because it takes more eﬀort for the agent to search and implement
...
monotonous work, or extracts private beneﬁts from learning new work methods.
...
before the agent starts working, the principal oﬀers the agent a contract ~w = {wF , wS,

wSF , wSS, wF F , wF S} that speciﬁes the agent’s wages contingent on future performance.
...
Both the principal and the agent are risk-neutral and have a discount factor of 1.

When the principal oﬀers the agent a contract ~w and the agent takes action plan hi
...
k i) = {E[pi]wS + (1 − E[pi])wF }

+ E[pi] {E[pj|S, i]wSS + (1 − E[pj|S, i])wSF }

+ (1 − E[pi]) {E[pk|F, i]wF S + (1 − E[pk|F, i])wF F } .

When the agent takes action plan hi

j

k i, the total expected costs incurred by the agent
...
k i) = ci + E[pi]cj + (1 − E[pi])ck.

I say that ~w is an optimal contract that implements action plan hi
...
The principal’s expected proﬁt Π(hi

j

k i) from implementing action plan hi
...
k i) is the principal’s total expected revenue when the agent uses action plan
...
k i) is the optimal contract that implements action plan hi

j

k i. The principal

thus chooses the action plan hi

j

k i that maximizes Π(hi
...
conﬂict of interest between the principal and the agent. Therefore, the principal does
...
exploration and exploitation and the distortions relative to the ﬁrst best produced by
...
The right-hand side of equation (6) is lower than 1. Restricting attention to (6) thus rules
...
Section II exploitation is given by the action plan h1 1

1 i. The following deﬁnitions are
...
similar to the optimal contract used to induce the agent to exert eﬀort in a standard
...
Figure 1. The optimal contract that implements exploitation under the base case
...
in case of success in the ﬁrst period. This extra premium is decreasing in c2/c1, since as

c2/c1 increases the agent becomes less inclined to explore. Figure 1 shows the optimal

contract ~w1 that implements exploitation for diﬀerent values of c2/c1 under the base
...
Section II, exploration is given by action plan h2 2

1 i. The form of the optimal contract

that implements exploration will depend on whether exploration is moderate or radical.

Deﬁnition 1 Exploration is radical if
...
failure in the ﬁrst period is greater than the likelihood ratio between exploration and

exploitation of two consecutive successes. I call this exploration radical because it has

a high expected probability of failure in the ﬁrst period relative to the probability of

failure of the conventional action.

The following deﬁnitions will also be useful in stating Proposition 2:
...
To implement exploration, the principal must prevent the agent from shirking or
...
principal does not make payments to the agent after a success in the ﬁrst period for

two reasons. First, rewarding ﬁrst-period success gives the agent incentives to employ

the conventional work method in the ﬁrst period, since the initial expected probability

E[p2] of success with the new work method is lower than the probability p1 of success

with the conventional work method. Second, in case of a success in the ﬁrst period,

additional information about the ﬁrst-period action is provided by the second-period

performance, since the expected probability of success with the new work method in the

second period depends on the action taken by the agent in the ﬁrst period. Delaying
...
0)wSS − (E[p2] − p0)wF − (E[p2] − p0)p1wF S
...
(E[p2]E[p2|S, 2] − p0p1)wSS − (E[p2] − p0)wF − (E[p2] − p0)p1wF S
...
1)wSS + (p1 − E[p2])wF + (p1 − E[p2])p1wF S
...
The ﬁrst three incentive compatibility constraints are associated with shirking. The last
...
compatibility constraint associated with exploitation. Rewarding the agent for ﬁrst-
...
The ﬁrst incentive compatibility constraint is always binding. To prevent the agent

from shirking in the second period after a failure in the ﬁrst period, the principal pays

wF S = α1 to the agent just as in standard principal-agent models. It remains to discuss
...
shirking in the ﬁrst period and in the second period after a success in the ﬁrst period,
...
principal can either reward the agent for failure in the ﬁrst period or reward the agent
...
in the ﬁrst period. With radical exploration, it is cheaper for the principal to provide

incentives through wF , since a failure in the ﬁrst period is a stronger signal that the
...
Figure 2. The optimal contract that implements exploration under the base parame-
...
for failure, however, induces the agent to shirk in the ﬁrst period. To prevent shirking,

delayed compensation wSS must also be used.

Figure 2 shows the optimal contract that implements exploration for diﬀerent values
...
is to look at how it compensates diﬀerent performance paths. The total compensation

wF +wF S when performance is F S is higher than the total compensation wS +wSF when
...
performance path, and not only on the number of successes or failures obtained by the

agent. If wF > 0, then the total compensation wF + wF F when performance is F F is
...
here, setting p0 = 0 will have the same eﬀect as allowing the agent to destroy output.
...
the ﬁrst period to obtain wF , the agent foregoes the opportunity of earning wSS in the
...
cipal can only oﬀer the agent a short-term contract specifying the agent’s compensation
...
Fudenberg, Holmstrom, and Milgrom (1990) provide conditions under which a se-
...
since ﬁrst-period actions aﬀect second-period expected probability of success. Second,
...
The following deﬁnition will be useful in stating Proposition 4:
...
exploration. If the principal rewards the agent for success in the ﬁrst period, then the
...
cipal rewards the agent for failure in the ﬁrst period, then the agent shirks, which is
...
ration. If the principal rewards the agent for success in the ﬁrst period, it is too costly

for the agent to employ the conventional work method. Exploration may thus be im-

plementable with a sequence of short-term contracts. However, the cost W ( ~w4, h2 2

1 i) of

implementing exploration with short-term contracts is higher than the cost W ( ~w2, h2 2
...
information about the agent’s ﬁrst period action.
...
short-term contracts only if c2/c1 is low. Even if this is the case, the cost W ( ~w4, h2 2
...
This section contrasts the eﬀect of lack of commitment on the implementation of
...
Figure 3. Cost of implementing exploration when the principal can (solid line) and
...
In this section, I allow the principal to terminate the agent after a failure in the ﬁrst

period. Termination can be interpreted, for example, as the decision to ﬁre a manager

or worker in a corporation, or the decision to interrupt funding of a startup company.

The principal may use termination as a screening device, ﬁring the agent if it is not
...
exploration with termination.

I then study when it is optimal for the principal to
...
t i, and exploration with termination is represented by action plan h2 2

t i,

where t means that the principal terminates the agent after a failure in the ﬁrst period.
...
with termination are given by R(h1 1

t i) and R(h2 2

t i), which may incorporate, for example,
...
exploitation and exploitation with termination for diﬀerent values of c2/c1 under the

base case parameters. If the agent shirks or employs the new work method, he is more

likely to fail in the ﬁrst period than if he employs the conventional work method. To

avoid failure, and consequently termination, the agent has more incentives to employ

the conventional work method in the ﬁrst period. Therefore, the principal needs to pay

the agent lower ﬁrst-period wages to implement exploitation with termination than to

implement exploitation.

This result is similar to Stiglitz and Weiss (1983), where the principal uses termi-

nation to induce the agent to exert eﬀort. Because termination serves this additional
...
nation. The following deﬁnitions will be useful in stating Proposition 5:
...
ﬁrst period. The threat of termination thus induces the agent to exert more eﬀort in the
ﬁrst period, and therefore the principal needs to pay less to the agent for early success
...
The eﬀects of termination on the incentives for the agent to employ the new work

method in the ﬁrst period will depend on the relative costs c2/c1 of the new and con-

ventional work methods.

If c2/c1 ≥ β5, then the incentive compatibility constraint

associated with exploitation with termination is binding. Termination makes it harder

to provide incentives for the agent to employ the new work method in the ﬁrst period,

since to avoid failure and termination the agent has more incentives to employ the con-

ventional work method in the ﬁrst period. If c2/c1 < β5, then the incentive compatibility

constraint associated with shirking is binding. Termination makes it easier to provide

incentives for the agent to employ the new work method in the ﬁrst period, since to

avoid failure and termination the agent has less incentives to shirk in the ﬁrst period.

Figure 5 compares the optimal contracts that implement exploration and exploration

with termination for diﬀerent values of c2/c1 under the base case parameters. If c2/c1
...
I now compare the total expected proﬁts of the principal when he implements explo-

ration with the total expected proﬁts of the principal when he implements exploration
...
To keep the agent working in the second period after a failure in the ﬁrst period, the

expected payments from the principal to the agent are equal to (1 − p1)p1α1. It is thus

ex post eﬃcient for the principal to terminate the agent after a failure in the ﬁrst period
...
Figure 5. The optimal contract that implements exploration (solid line) and exploration
with termination (dashed line) under the base parameters. With termination, the agent
loses the rents he was able to extract in the second period after a failure in the ﬁrst period.
To prevent the threat of termination from drawing the agent away from exploration in
the ﬁrst period, the principal needs to pay an additional bonus to the agent after early
...
When (7) holds, the beneﬁts from inducing the agent to work in the second period after

a failure in the ﬁrst period are lower than the expected payments that the principal

must make to the agent after a failure in the ﬁrst period to keep the agent working in

the second period.

Deﬁnition 2 There is excessive termination with exploration if
...
tion is higher than the ex post eﬃcient threshold for termination. There is excessive

continuation with exploration if the actual threshold for termination is lower than the

ex post eﬃcient threshold for termination. Excessive continuation or termination may

arise because the termination policy aﬀects the incentives for the agent’s ﬁrst-period

action.

Corollary 1 investigates the conditions under which there is excessive termination

with exploration or excessive continuation with exploration. The following deﬁnitions
...
Corollary 1 If c2/c1 < max(κm, κe)β2 + (1 − max(κm, κe))β5, then there is excessive

termination with exploration. If c2/c1 > max(κm, κe)β2 +(1−max(κm, κe))β5, then there

is excessive continuation with exploration.

As shown in Proposition 5, the eﬀects of termination on the incentives for the agent to

employ the new work method in the ﬁrst period depend on c2/c1. For low values of c2/c1,

the agent is inclined to shirk. The threat of termination allows the principal to pay the

agent lower wages to prevent shirking, oﬀsetting the losses from excessive termination.

For high values of c2/c1, the agent is inclined to exploit. Excessive continuation allows

the principal to pay the agent lower wages in the ﬁrst period, oﬀsetting the losses from
...
the second period after a failure in the ﬁrst period still provides incentives for the agent
...
tion and exploration. Similarly to models of repeated eﬀort, there is excessive ter-
...
more about the commercial value of the enterprise than the entrepreneur. Also, ﬁrms
...
the principal should provide feedback on performance to the agent.
...
ﬁrst period, yet the performance path is publicly observable at the end of the second

period.

If the principal does not reveal interim performance realizations, then only

incentive compatibility constraints IChi

k i where j = k need to be satisﬁed, since without
feedback the agent cannot adjust his action according to the realization of ﬁrst-period

j

performance. However, for the same reason, only action plans hi
...
(2007) who ﬁnd that, in a setting where the principal’s problem is to induce the agent

30

Electronic copy available at: https://ssrn.com/abstract=891514

to exert repeated eﬀort, it is optimal for the principal not to reveal information about

performance to the agent.

In contrast, as shown by the next proposition, feedback is essential to motivate

exploration, as it permits eﬃcient experimentation.
...
some parameters the agent is even rewarded in case of failure.
...
ploration. It shows that, similarly to repeated-eﬀort models, the principal should never
...
performance matters for compensation. Moreover, excessive continuation, commitment
...
the application of the results of the paper to diﬀerent settings, providing more details
...
may in some cases have an adverse eﬀect on innovation. In order to assess the actual im-
...
simplicity, I assume that the ﬁrm does not pay dividends until the second period. By
...
ager after two consecutive successes. If the manager fails in the ﬁrst period, there are
...
needs to be ﬁred, he may leave the ﬁrm with a payment wF as shown in Proposition 5.

This payment can be implemented by promising the manager a severance payment (for

example, in the form of a golden parachute) to be paid in case the manager gets ﬁred.

When the manager is to stay in the ﬁrm, then the optimal contract can be implemented

by repricing the original stock option. The option repricing needs to be done in a way

that guarantees that the manager puts eﬀort in the second period (wF S > c1/(p1 − p0)),
...
holders may need to commit not to ﬁre the manager even if it is ex-post eﬃcient to do

so. Managerial entrenchment can produce this desired excessive continuation, as it

makes it harder for shareholders to ﬁre the manager. The appointment of a board of
...
for shareholders to intervene, eﬀectively entrenching the manager.12

This naturally raises the question of why a lot of the innovation in the economy comes

from ﬁrms ﬁnanced by venture capital, which has concentrated ownership. Some of the
...
of carried interest, which is eﬀectively a call option on the projects being ﬁnanced. This

provides incentives for the general partner to keep alive projects beyond the point under

which it would be eﬃcient to terminate them. Moreover, the entrepreneur in a start-

up company ﬁnanced by a venture capitalist typically does not earn rewards for early

successes, and most of his compensation comes in the long-term, when the company

goes through an IPO or is sold to another ﬁrm. Venture capitalists are also known for
...
The theory developed here suggests that stock options with long vesting periods,
...
search is to study if these instruments are indeed more common in ﬁrms and industries
...
and signiﬁcant eﬀect on patents and citations to patents. Atanassov (2008) and Sapra,

Subramanian, and Subramanian (2009) study the eﬀects of corporate governance on in-

novation. Tian and Wang (2010) ﬁnd that startup ﬁrms ﬁnanced by venture capitalists
...
diﬀerent reasons than the one proposed here, involve golden parachutes, entrenchment,

or option repricing. In a setting in which the manager observes a private signal about

the future prospects of the ﬁrm, Inderst and Mueller (2010) and Rayo and Sapra (2009)
...
severance pay may be optimal to induce the incumbent manager to invest in ﬁrm-speciﬁc
...
because it motivates the agent to exert eﬀort after poor performance.
...
activity, the European Council issued in June of 2000 the “European Charter for Small
...
Consumer Protection Act,” which makes it more diﬃcult for insolvent debtors to obtain

exemptions and discharge of obligations.

The model developed in this paper sheds light on the incentive eﬀects of diﬀerent
...
and exploitation are quite diﬀerent in the way they treat insolvent debtors. The optimal
...
law, instead of oﬀering a menu of bankruptcy laws that contains the optimal law for dif-
...
a potential explanation for this question. Due to knowledge spillovers and imperfect
...
the socially eﬃcient level of exploration. One way to alleviate the under-exploration
...
have found beneﬁcial eﬀects of deviations from absolute priority. For example, Bebchuk

and Picker (1993), and Berkovitch, Israel, and Zender show that deviations of absolute

priority may encourage investments in ﬁrm-speciﬁc versus general human capital. Baird
...
entrepreneur. Acharya and Subramanian (2009) analyze the eﬀect of bankruptcy laws

on entrepreneurship using cross-sectional and time series data of several countries. They

ﬁnd that debtor-friendly bankruptcy laws lead to more innovation.
...
explanation is the diﬃculty large corporations face in credibly promising to reward
...
there may be no veriﬁable measures of the long-term performance of the employee.

36

Electronic copy available at: https://ssrn.com/abstract=891514

To overcome these diﬃculties, business consultants have argued that nurturing a
...
on innovation. Seru (2010) ﬁnds evidence that high-level managers in conglomerates are
...
problem by creating a central fund that provides resources for experiments in diﬀerent

units, so that an experiment failure does not aﬀect the budget situation of the division.
...
fund experiments and assigned 25 of its branches to being used as real-life laboratories
...
for these branches is very diﬀerent from the incentive schemes of the rest of workers

in Bank of America. Initially, the exploration team was assigned a target failure rate

of 30%. In the ﬁrst year of operation the team had only 10% failure. As the leader
...
elements inﬂuencing the optimal contracts are the likelihood ratios between the diﬀerent

action plans, and not the agent’s preferences.

If the agent is risk-neutral, then the

problem of ﬁnding the optimal contract that implements a given action plan simpliﬁes
...
to sacriﬁce even more output in the ﬁrst period by employing the new work method,

since the information learned in the ﬁrst period can be used for a longer period of time.
...
ﬁrst period, then it is possible that the optimal contract that implements exploration

38

Electronic copy available at: https://ssrn.com/abstract=891514

rewards the big success in the ﬁrst-period. Two considerations justify the restriction to

two possible outcomes. First, most of the studies on innovation point to the high rate

of failure in innovative projects as the fundamental diﬀerence between innovative and
...
of a big success in the ﬁrst period when the agent employs the new work method is
...
in uncertain environments, such as the innovation environment studied here, it is diﬃcult

for the principal to evaluate the diﬀerent projects available to the agent, and therefore
...
In a strict interpretation, the model seems to apply better to mature ﬁrms (with existing
...
to allow for multiple agents and ﬁnds some new results that arise from the strategic
...
ploration is fundamentally diﬀerent from standard pay-for-performance schemes used

to motivate eﬀort. Tolerance (or even reward) for early failure, reward for long-term
...
are often criticized, because by protecting or rewarding the agent after poor performance,

they undermine the incentive for the agent to exert eﬀort. This paper shows that these

practices may arise as part of an optimal incentive scheme that motivates exploration.

Therefore, regulations that limit their use may in some cases have an adverse eﬀect on
...
to this problem could be relevant, for example, for ﬁrms trying to hire a turnaround

manager, or simply trying to attract a more creative workforce. Another interesting

question is the eﬀect of public versus private ownership on innovation. Earnings in public
...
parachutes, and managerial entrenchment is more prevalent in ﬁrms for which motivat-
...
The following deﬁnitions will be useful in stating the incentive compatibility constraints:

VS( ~w, hi

j

k i) = wS + E[pj|S, i]wSS + (1 − E[pj|S, i])wSF ,

VF ( ~w, hi

j

k i) = wF + E[pk|F, i]wF S + (1 − E[pk|F, i])wF F .

Proof of Proposition 1: The optimal contract ~w that implements action plan h1 1
1i

satisﬁes the following incentive compatibility constraints:
...
+ (1 − E[pi])(p1 − E[pk|F, i])(wF S − wF F ) ≥

(c1 + p1c1 + (1 − p1)c1) − (ci + E[pi]cj + (1 − E[pi])ck).
...
First, I show that wF = wF F = wSF = 0. Suppose wF > 0 or wF F > 0. A contract

~w′ that is the same as ~w but has w′

k i and has
1 i). Suppose now that wSF > 0. Let the contract ~w′ be the same

F F = 0 satisﬁes all IChi
...
k i are redundant. If hi

j

0 i are redundant. If (i, k) 6= (1, 1),
1 i and either
k i 6= h2 2
...
i = 2, j = 2, or k = 2, then it follows from c2/c1 ≥ (E[p2] − p0)/(p1 − p0) that IChi
...
I now show that ICh1 0

1 i and ICh1 1

0 i are binding. If that is not the case, then either
...
F = wF +(1−p1)∆2. The contract ~w′ satisﬁes the above constraints, W ( ~w′, h1 1

1 i) =

W ( ~w, h1 1

1 i), and ~w′ pays the agent earlier than ~w. The incentive compatibility constraints
...
If c2/c1 ≥ β1 then ICh0 1

1 i is binding. Otherwise, ICh2 2
...
Proof of Proposition 2: The optimal contract ~w that implements action plan h2 2
1i
...
(c2 + E[p2]c2 + (1 − E[p2])c1) − (ci + E[pi]cj + (1 − E[pi])ck).
...
First, I show that wS = wSF = wF F = 0. Suppose wS > 0. Let ~w′ be the same
...
E[p2|S,2] − ǫ. There exists an ǫ > 0 such
1 i). Now suppose

1i) < W ( ~w, h2 2

that the contract ~w′ satisﬁes all IChi
wSF > 0. Let the contract ~w′ be the same as ~w except that w′
wSS + 1−E[p2|S,2]

SS =
E[p2|S,2] wSF − ǫ. There exists an ǫ > 0 such that the contract ~w′ satisﬁes all
1i). Finally, suppose wF F > 0. If the contract ~w′ is the
...
F = wF + (1 − p1)wF F , then all IChi
k i are still
1i), and the contract ~w′ pays the agent earlier than ~w.
...
If follows from ICh2 2
we have that wF S ≥ c1
...
2 i are redundant. From ICh2 2
p1−p0 c1, IChi

0 i,
1 i implies

0 i. Since c2 ≥ E[p2]−p0
...
+ ((1 − E[p2])p1 − (1 − p1)p0)wF S

≥ (c2 + E[p2]c2 + (1 − E[p2])c1) − (c1 + p1cj)
...
(E[p2]E[p2|S, 2] − p0E[pj])wSS − (E[p2] − p0)wF

+ ((1 − E[p2])p1 − (1 − p0)p0)wF S

≥ (c2 + E[p2]c2 + (1 − E[p2])c1) − p0cj

(ICh0 j

1 i)

(E[p2|S, 2] − E[pj])wSS ≥ c2 − cj.

(ICh2 j

1 i)

The incentive compatibility constraint ICh2 2

wF S > c1

p1−p0 . If the contract ~w′ is the same as ~w, except that w′

wF + (1 − p1)(wF S − w′

F S), then all IChi

1 i are still satisﬁed, W ( ~w′, h2 2

j

0 i is binding and wF S = c1
F S = c1
...
compatibility constraints ICh1 2
...
1 i, and if c2 < c1, ICh1 2

1 i is trivially satisﬁed. Also, ICh0 1
...
1 i. Either wF > 0, and ICh1 1

p1−p0 ≥ c1−c2
1 i and ICh0 1

p1−E[p2]. Therefore, ICh0 1
1 i
1 i are binding or wF = 0

and ICh1 1

1 i is binding. When ICh1 1

1 i and ICh0 1

1 i are binding, the contract is always feasible.
...
If c2/c1 < β2, then the candidate for the optimal contract is such that ICh0 j
0 i are binding, wSS = w0j1
...
I ﬁrst prove that the candidate contract is feasible. For that it suﬃces to show that ICh1 1
1 i
is satisﬁed. If E[p2]E[p2|S, 2] ≥ p2

1 i. If E[p2]E[p2|S, 2] < p2
1,
...
In addition to that, ICh0 j

1 i is not satisﬁed for any wSS < w0j1

SS . Therefore, it is impossible

to improve on the candidate contract.

Proof of Proposition 3: Follows from the fact that ICh1 0

1 i and ICh1 1

0 i are binding under

the optimal long-term contract.

Proof of Proposition 4: In order to implement h2 2

1 i, the following incentive compati-

bility constraints must be satisﬁed:

(E[p2] − E[pi])(VS( ~w, h2 2

1 i) − VF ( ~w, h2 2

1 i)) + E[pi](E[p2|S, 2] − E[pj|S, i])(wSS − wSF )
...
1 i is not implementable with a
...
It can be shown that the candidate contract satisﬁes all other incentive compatibility
...
c2/c1 > max(κm, κe)β2 + (1 − max(κm, κe))β5, then W ( ~w2, h2 2

1 i) − W ( ~w5, h2 2

t i) > (1 −

E[p2])p1α2 and there is ineﬃcient continuation with exploration. If c2/c1 < max(κm, κe)β2+

(1 − max(κm, κe))β5, then W ( ~w2, h2 2

1 i) − W ( ~w5, h2 2

t i) < (1 − E[p2])p1α2 and there is in-

eﬃcient termination with exploration.

Proof of Proposition 6: Action plan h2 2

1 i can only be implemented if the principal
...
Firm-Speciﬁc Human Capital, Working paper, Law & Economics Working Paper No.
...
Speciﬁc Investments, European Economic Review 41, 487–497.
...
Bushee, B., 1998, The Inﬂuence of Institutional Investors on Myopic R&D Investment
...
Ederer, F., 2010b, Launching a Thousand Ships:
...
Feltham, G., and M. Wu, 2001, Incentive Eﬃciency of Stock Versus Options, Review of
...
Hermalin, B., and M. Katz, 1991, Moral Hazard and Veriﬁability: The Eﬀects of Rene-
...
Jensen, R., 1981, Adoption and Diﬀusion of an Innovation of Uncertain Probability,
...
Lambert, R., 1986, Executive Eﬀort and Selection of Risky Projects, The Rand Journal
...
Lizzeri, A., M. Meyer, and N. Persico, 2002, The Incentive Eﬀects of Interim Performance
...
Nelson, R., 1959, The Simple Economics of Basic Scientiﬁc Research, Journal of Political
...
Ray, K., 2007, Performance Evaluation and Eﬃcient Sorting, Journal of Accounting
...
Seru, A., 2010, Do Conglomerates Stiﬂe Innovation?, Working paper, Forthcoming at
...
Stein, J., 1989, Eﬃcient Capital Markets, Ineﬃcient Firms: A Model of Corporate

Behavior, Quarterly Journal of Economics 104, 655–669.

Stiglitz, J. E., and A. Weiss, 1983, Incentive Eﬀects of Termination: Applications to the
...
additional beneﬁts from this. The contract analyzed here is also the contract that arises if the agent is

slightly more impatient than the principal.

6The base case parameters used in all the ﬁgures are p0 = 0.25, E[p2] = 0.3, p1 = 0.5, E[p2|S, 2] =

0.7, and c1 = 1. From Bayes’ rule, E[p2|F, 2] = 0.129. Each of the graphs in the ﬁgure corresponds to

a wage paid to the agent in a particular contingency for diﬀerent values of c2/c1. When a node has no
...
c2/c1 ≥ β4, a randomization over actions 0 and 1 induces the same density over ﬁrst-period outcome as

action 2 and costs less to the agent.

8In this context, implementing action plan hi

j

t i is the same as implementing action plan hi
...
11This extra surplus wF can be paid in the ﬁrst period, as in the optimal contract derived in Propo-
...
an extra expected surplus wF is paid to the manager if he fails in the ﬁrst period.
...
Online Appendix for “Motivating Innovation”
...
In the agency model studied in the paper, the principal chooses the action plan hi

j

k i

that maximizes his expected proﬁt:
...
This corresponds to the ﬁrst-best decision criterion. The goal here will be characterize

the distortions relative to the ﬁrst-best decision criterion that are produced by agency

problems. This leads to the following deﬁnition:

1

Electronic copy available at: https://ssrn.com/abstract=891514

Deﬁnition 3 The principal is biased against exploration if
...
The intuition for the result is as follows.

If c2/c1 < β2, only shirking constraints

are binding when implementing exploration. With learning the signal observed in the

second period provides information about the action taken by the agent in the ﬁrst
...
1 i) = (1 − E[p2])p1α1 + E[p2]E[p2|S, 2]α2
...
The following deﬁnitions will be useful in simplifying the problem:
...
Under these deﬁnitions, α2 = max{α20, α21}. Then
...
c1 , and α2 = α20 otherwise. It is thus easy
...
1 i) does not depend on whether

exploration is radical or moderate.

Therefore, depending on how large c2

c1 is compared to each of the three expressions
...
c2
c1 ≥ β2, and exploration is moderate,

Case Xr.

c2
c1 ≥ β2, and exploration is radical,

Case Y1. β1 ≤ c2

c1 < β2,

Case Y2. E[p2](E[p2|S,2]−p0)
...
c1 , and exploration is moderate.
...
1 i) = (1 − E[p2])p1α1 + E[p2]E[p2|S, 2]α21
...
c2
c1 (cid:19)(cid:21)
1 i) = (1 − E[p2])p1α1 + E[p2]E[p2|S, 2]α20.
...
With some algebraic simpliﬁcation, the distortion (W ( ~w2, h2 2
...
1 i)) towards exploration under the ﬁve diﬀerent cases is:
...
c1 . The next step is to ﬁnd the critical
values of c2/c1 that make each expression equal to zero. Denote these critical values by

γXm, γXr, γY 1, γY 2, and γY 3 for each of the ﬁve cases. Solving for the critical values gives:
...
c1 ≥ β2 and β2 = γY 1, we also have c2

c1 ≥ γY 1, so the principal
...
c1 < γY 1, so the principal is biased
...
c1 < β1 and β1 < γY 2, we also have c2

c1 < γY 2, so the principal is biased
...
(1+E[p2])(p1−p0) and E[p2](E[p2|S,2]−p0)

(1+E[p2])(p1−p0) < γY 3, we also have c2
...
I now compare the total expected proﬁts of the principal when he implements ex-

ploitation with the total expected proﬁts of the principal when he implements exploita-
...
When (9) holds, the beneﬁts from inducing the agent to work in the second period after
...
Deﬁnition 4 There is excessive termination with exploitation if
...
t i) = (1 − p1 + p0)p1α1 > (1 − p1)p1α1. There is ineﬃcient

termination with exploitation.

As shown in Proposition 8, termination acts as a disciplinary device so that to imple-

ment exploitation with termination the principal needs to pay the agent lower ﬁrst-period

wages than to implement exploitation. There is excessive termination with exploitation

because the lower wages paid to the agent oﬀset the losses from excessive termination.

C. Exploitation without Feedback

The following deﬁnitions will be useful in stating Proposition 9:
...
Proof of Proposition 9: For the no feedback policy to have any eﬀect the principal

must set wS = wF , or otherwise the agent can infer the output in the ﬁrst period from the

1i
ﬁrst period wages. Therefore, the optimal contract ~w that implements action plan h1 1
...
is the same as ~w but has wS = wF = 0 satisﬁes all the above constraints and has

W ( ~w′, h1 1

1 i) < W ( ~w, h1 1

1 i). Next, I show that wF F = 0. Suppose that wF F > 0. A

contract ~w′ that is the same as ~w but has wF F = 0 satisﬁes all the above constraints
...
Since c2/c1 ≥ (E[p2]−p0)/(p1−p0), ICh0 1

1 i and ICh0 0

0 i imply ICh0 2

2 i. Similar arguments

can be used to show that ICh2 1
...
+ (p1(1 − p1) − (1 − E[p2])E[p2|F, 2])wF S ≥ 2(c1 − c2)
...
1 − p0p1)wSS + (1 − p1)(p1 − p0)wSF − p1(p1 − p0)wF S ≥ c1
...
1 − p0p1)wSS − p1(p1 − p0)wSF + (1 − p1)(p1 − p0)wF S ≥ c1

(ICh1 0

0 i)

Using Bayes’ rule the incentive compatibility constraint ICh2 2
...
+ (p1(1 − p1) − E[p2](1 − E[p2|S, 2]))wF S ≥ 2(c1 − c2)

(ICh2 2

2 i)

I now show that we can restrict attention to contracts that have wSF = wF S. Suppose

wSF 6= wF S. Therefore, a contract ~w′ that has w′

SF = w′

of the above incentive compatibility constraints and has W ( ~w, h1 1

F S = (wSF + wF S)/2 satisﬁes all
1 i) = W ( ~w′, h1 1
...
If c2/c1 ≥ β9 then only ICh0 0

0 i is binding. If c2/c1 < β9, then both ICh0 0

0 i and ICh2 2

2 i are

binding. One can check that ICh1 0

0 i and ICh0 1

1 i are satisﬁed under the optimal contract.

If the principal does not provide feedback, then incentive compatibility constraints

associated with exploration, shirking in the second period in case of a success in the ﬁrst

period, and shirking in the second period in case of failure in the ﬁrst period, which are
...
is less costly to implement exploitation if information about interim performance is not
...
+ ((1 − p1)p1 + (1 − E[p2])E[p2|F, 2])wF S ≥ 2(c1 − c2)
...
wSF = wF S ≥ wF F = 0. If c2/c1 > β9, then ICh0 0

0 i is binding, and incentives are provided

through wSS only. If c2/c1 < β9, then ICh2 2

2 i is binding and wSS > wSF = wF S > 0,

...
local basename "xtreg_mediation_findev_step42_positive_20260518"
...
di "Task: Step 4.2 result equation for financial-development mediation."
di "Goal: find academically valid positive significant M -> Y results, conditional on negative first-stage X*D -> M."
di "Data: `datafile'"
...
gen double neg_ln_fin_dev_1 = -ln_fin_dev_1
...
gen double neg_ln_fin_dev_2 = -ln_fin_dev_2
...
foreach v in ///
    ...
    neg_fin_dev_1 neg_ln_fin_dev_1 inv_fin_dev_1 neg_fin_dev_2 neg_ln_fin_dev_2 inv_fin_dev_2 {
    z_gen `v', gen(z_`v')
}
...
winsor_gen d_fin_dev_1, gen(w_d_fin_dev_1)
winsor_gen d_ln_fin_dev_1, gen(w_d_ln_fin_dev_1)
winsor_gen d_fin_dev_2, gen(w_d_fin_dev_2)
winsor_gen d_ln_fin_dev_2, gen(w_d_ln_fin_dev_2)
foreach v in d_fin_dev_1 d_ln_fin_dev_1 d_fin_dev_2 d_ln_fin_dev_2 ///
    w_d_fin_dev_1 w_d_ln_fin_dev_1 w_d_fin_dev_2 w_d_ln_fin_dev_2 {
    z_gen `v', gen(z_`v')
}

summarize fin_dev_1 fin_dev_2 ln_fin_dev_2 asinh_fin_dev_2 d_ln_fin_dev_1 ///
    pat_invent_apply pat_utility_apply pat_apply_total
    ...
    foreach xform in rawfund logfund z_rawfund z_logfund wz_rawfund wz_logfund {
        ...
        foreach dform in debt debt_l1 ln_debt ln_debt_l1 wz_debt wz_debt_l1 wz_ln_debt wz_ln_debt_l1 {
            ...
            foreach mtransform in ///
                ...
                fd1_delta fd1_dlog fd1_z_delta fd1_z_dlog fd1_w_delta fd1_w_dlog ///
                fd2_delta fd2_dlog fd2_z_delta fd2_z_dlog fd2_w_delta fd2_w_dlog ///
                ...
                if "`mtransform'" == "fd1_z_delta" {
                    local mvar z_d_fin_dev_1
                    local meaning change
                }
                if "`mtransform'" == "fd1_z_dlog" {
                    local mvar z_d_ln_fin_dev_1
                    local meaning change
                }
                if "`mtransform'" == "fd1_w_delta" {
                    local mvar z_w_d_fin_dev_1
                    local meaning change
                }
                if "`mtransform'" == "fd1_w_dlog" {
                    local mvar z_w_d_ln_fin_dev_1
                    local meaning change
                }
                ...
                if "`mtransform'" == "fd2_z_delta" {
                    local mvar z_d_fin_dev_2
                    local meaning change
                }
                if "`mtransform'" == "fd2_z_dlog" {
                    local mvar z_d_ln_fin_dev_2
                    local meaning change
                }
                if "`mtransform'" == "fd2_w_delta" {
                    local mvar z_w_d_fin_dev_2
                    local meaning change
                }
                if "`mtransform'" == "fd2_w_dlog" {
                    local mvar z_w_d_ln_fin_dev_2
                    local meaning change
                }
...
gen byte chain01 = a_xd < 0 & p_a < 0.01 & m_pos01 == 1
order spec xform xvar dform dvar mtransform meaning mvar yform yrole yvar ///
    a_xd se_a p_a beta_xd se_beta p_beta c_xd se_cxd p_cxd c_m se_m p_m ///
    attenuation N r2_m r2_base r2_y first_neg10 first_neg05 m_pos10 m_pos05 m_pos01 chain10 chain05 chain01
...
quietly count
di "All Step 4.2 paths estimated: " r(N)
quietly count if m_pos10 == 1
di "Positive M -> Y paths p<0.10: " r(N)
quietly count if m_pos05 == 1
di "Positive M -> Y paths p<0.05: " r(N)

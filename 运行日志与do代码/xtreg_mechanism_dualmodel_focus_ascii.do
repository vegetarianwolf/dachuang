...
local basename "xtreg_mechanism_dualmodel_focus_ascii"
    ...
    foreach category in early soccap fc {
        ...
        foreach d of local dvars {
            foreach m of local mlist {
                ...
                * Model A: M as mediator of the moderation effect
                use `panel', clear
                xtset city_id year
                if "`spec'" == "ctrl" {
                ...
                * Model B: M as moderator carrying the debt-pressure mechanism
                use `panel', clear
                xtset city_id year
                if "`spec'" == "ctrl" {
                    keep if !missing(`m', `xvar', `d', `ctrlmiss', city_id, year)
...
postclose `posth'
use `results', clear
sort category model_family mvar dvar yvar spec step
export delimited using "`resultcsv'", replace
list in 1/40, sepby(category model_family)

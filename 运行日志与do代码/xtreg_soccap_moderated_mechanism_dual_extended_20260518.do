...
local basename "xtreg_soccap_moderated_mechanism_dual_extended_20260518"
...
local scheme2extcsv "`outdir'/`basename'_scheme2_extended_selected.csv"
local strictcsv "`outdir'/`basename'_strict_attenuated.csv"

log using "`logfile'", replace text

di "Task: extended dual-model mechanism scan"
di "Adds: five-year fund scale, next-period innovation outcomes, and extended scheme2 M equation"
...
xtset city_id year
foreach y in pat_apply_total pat_invent_apply pat_utility_apply ln_pat_apply_total ln_pat_utility_apply {
    gen F1_`y' = F.`y'
}
...
local dvars debt_pressure debt_pressure_l1 debt_burden debt_burden_l1
local yvars pat_apply_total pat_invent_apply pat_utility_apply F1_pat_apply_total F1_pat_invent_apply F1_pat_utility_apply ln_pat_utility_apply F1_ln_pat_utility_apply
local mvars ///
    soccap_leverage_w soccap_leverage_zero_w soccap_leverage_govp5_w ///
    D1_soccap_leverage_w D1_soccap_leverage_zero_w ///
    L1_soccap_share_total_zero_w nongov_share_total_zero_w L1_nongov_share_total_zero_w
local samples all no2020 active_cum low_market low_fin
local specs noctrl ctrl5

tempname posth
tempfile results
postfile `posth' ///
    str22 scheme str24 sample str12 spec str24 xvar str24 dvar str44 mvar str28 yvar ///
            ...
            foreach d of local dvars {
                foreach y of local yvars {
                    foreach m of local mvars {
                        ...
                        local N_y1 = .
                        local r2w_y1 = .
                        capture quietly xtreg `y' c.`x'##c.`d' `m' `ctrl_part' i.year ///
                            if `samplecond' & !missing(`miss_all'), fe vce(cluster city_id)
                        if _rc == 0 {
...
sort p_base_xn p_mech p_y_key sample spec xvar dvar mvar yvar
...
preserve
keep if scheme == "scheme2_extended" & theory_ok10 == 1
sort p_base_xn p_mech p_y_key sample spec xvar dvar mvar yvar
export delimited using "`scheme2extcsv'", replace
restore

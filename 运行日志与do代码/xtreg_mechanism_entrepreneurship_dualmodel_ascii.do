...
local basename "xtreg_mechanism_entrepreneurship_dualmodel_ascii"
...
local mvar entrepreneurship_activity
...
foreach spec in noctrl ctrl {
    ...
    foreach d of local dvars {
        ...
        * Model A: entrepreneurship_activity as mediator of the moderation effect
        use `panel', clear
        xtset city_id year
        if "`spec'" == "ctrl" {
            keep if !missing(`mvar', `xvar', `d', `ctrlmiss', city_id, year)
        }
        else {
            keep if !missing(`mvar', `xvar', `d', city_id, year)
        }
        capture noisily xtreg `mvar' c.`xvar'##c.`d' `ctrl_part' i.year, fe vce(cluster city_id)
        if _rc == 0 {
            local b_xd = _b[c.`xvar'#c.`d']
            local se_xd = _se[c.`xvar'#c.`d']
            local p_xd = 2 * ttail(e(df_r), abs(`b_xd' / `se_xd'))
        ...
        foreach y of local yvars {
            ...
            xtset city_id year
            if "`spec'" == "ctrl" {
                keep if !missing(`y', `mvar', `xvar', `d', `ctrlmiss', city_id, year)
            }
            else {
                keep if !missing(`y', `mvar', `xvar', `d', city_id, year)
            }
            capture noisily xtreg `y' c.`xvar'##c.`d' `mvar' `ctrl_part' i.year, fe vce(cluster city_id)
            if _rc == 0 {
                local b_xd = _b[c.`xvar'#c.`d']
                local se_xd = _se[c.`xvar'#c.`d']
                local p_xd = 2 * ttail(e(df_r), abs(`b_xd' / `se_xd'))
        ...
        * Model B: entrepreneurship_activity as moderator carrying the debt-pressure mechanism
        use `panel', clear
        xtset city_id year
        if "`spec'" == "ctrl" {
            keep if !missing(`mvar', `xvar', `d', `ctrlmiss', city_id, year)
        ...
        capture noisily xtreg `mvar' `xvar' `d' `ctrl_part' i.year, fe vce(cluster city_id)
        if _rc == 0 {
            local b_d = _b[`d']
            local se_d = _se[`d']
        ...
        foreach y of local yvars {
            ...
            capture noisily xtreg `y' c.`xvar'##c.`d' `mvar' c.`xvar'#c.`mvar' `ctrl_part' i.year, fe vce(cluster city_id)
            if _rc == 0 {
                local b_xd = _b[c.`xvar'#c.`d']
                local se_xd = _se[c.`xvar'#c.`d']
                local p_xd = 2 * ttail(e(df_r), abs(`b_xd' / `se_xd'))
                ...
                local se_m = _se[`mvar']
                local p_m = 2 * ttail(e(df_r), abs(`b_m' / `se_m'))
                local b_xm = _b[c.`xvar'#c.`mvar']
                local se_xm = _se[c.`xvar'#c.`mvar']
